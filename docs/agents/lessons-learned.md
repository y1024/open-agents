...
- The installed AI SDK supports MCP clients and provider-native MCP/server bindings, but this repo's current agent runtime still uses a static tool registry; adding automation MCP support will require explicit runtime wiring, not just UI-level connection state.
- Automation schedule helpers (`getAutomationNextRunAt`, cron summary/preview utilities) throw synchronously on invalid cron/timezone input; API routes must catch those errors and return 400s instead of assuming Zod shape validation already guarantees a runnable schedule.
- Automation definitions should be soft-deleted rather than hard-deleted; otherwise foreign-key cascades remove `automation_runs` history and run-history/session links lose their source automation context.
...
- Local `next dev` must apply pending Drizzle migrations before serving the app; otherwise schema additions can surface as runtime query errors (for example missing `sessions` columns) even though `build` would have migrated successfully.
